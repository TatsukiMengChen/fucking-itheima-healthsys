...
**班 级**: 2024级
**完成人数**: 1
**起讫日期**: 2025.06.23 - 2025.07.05
**完成时间**: 2025.07.05

### 1\. 项目概述

通过本次软件开发实训，旨在指导学生使用 Java 语言基于 Java Swing 框架和数据库编程实现一个具有一定规模和复杂度的软件开发项目，以提高学生编写、调试、测试程序、运行维护及文档撰写等软件开发能力。本项目聚焦于“健康管理系统”的开发，旨在深入理解结构化和面向对象软件开发过程全生命周期的基本设计、开发方法和技术。

该系统将显著优化医疗资源配置，大幅节省人力、物力及时间成本。它不仅为医护人员提供高效的健康数据管理平台，实现检查项目的精准追踪和患者信息的动态管理，更为普通用户带来便捷的健康自检服务。通过可视化数据面板，系统使健康监测从传统的被动诊疗转变为主动预防，真正实现“让健康管理触手可及”的服务理念，全面提升医疗服务的可及性和精准性。

### 2\. 训练目的与要求

**训练目的**:
通过指导学生使用 Java 语言基于 Java Swing 框架和 MySQL 数据库编程实现一个具有一定规模和复杂度的软件开发项目，提高学生编写程序、调试程序、软件测试、运行维护及文档撰写等软件开发能力。使学生掌握结构化和面向对象软件开发过程全生命周期的基本设计、开发方法和技术，了解影响设计目标和技术方案的各种因素，能够选择与使用恰当的结构化和面向对象软件开发过程中需用到的软件工程工具、信息资源，主要针对桌面应用和 C/S 结构系统中的复杂软件工程问题，进行复杂软件系统的分析、设计、实现、验证、应用和维护。

**训练内容和要求**:
要求学生根据老师给定的选题和需求描述，1 人 1 组，独自完成需求分析，程序模块设计以及程序的编写、调试和测试。要求使用 Java 语言基于 Java Swing 框架实现一个桌面应用程序。

### 3\. 需求分析

根据大作业任务书和项目初期描述，本健康管理系统需满足以下核心需求:
...
7.  **权限分配功能（管理员、普通用户）**: 本系统将简化为普通用户、管理员、超级管理员三级权限。
8.  **用户信息的注册、验证、登录功能。**
9.  **检查项的增删改查。**
10. **检查组的增删改查。**
11. **系统运行安全稳定且响应及时。**
12. **高可用架构：MySQL 事务保障数据一致性，连接池优化确保秒级响应。** (**注：根据之前约定，数据库将采用 PostgreSQL。** )
...
* `CheckItemTableComponent` (View):
    ...
    * **ViewModel**: `CheckItemManagementViewModel`。
        ...
        * **逻辑**: `searchCommand` 调用 `CheckItemService.queryAllCheckItem()` 方法进行单条件或多条件查询。`addCommand`、`editCommand`、`deleteCommand` 调用 `CheckItemService` 进行相应操作。
        * **错误处理**: 校验输入合法性，操作失败时提示。
* `CheckItemEditFormComponent` (View):
    * **UI 元素**: 代号、名称、参考值、单位输入框，提交按钮。
    * **交互**:
        * 用户填写或修改检查项信息。
        * 点击“提交”按钮。
    * **ViewModel**: `CheckItemEditViewModel`。
        ...
        * **命令**: `submitCommand`。
        * **逻辑**: `submitCommand` 判断是新增还是修改。新增时调用 `CheckItemService.addCheckItem()`，需检查代号是否重复。修改时调用 `CheckItemService.updateCheckItem()`。
        * **校验**: 字段非空和长度校验（Hutool `Validator` 辅助或 `SystemVerifier` 模式）。
...
**递交时间**:
在设计完成之后书写设计说明书（实验报告），按规定报告的格式书写，在2025年7月5日前与设计相关文档（包括源代码）一并上交。

**考核方法**:
项目演练实验（40%）+真实项目开发大作业报告和答辩（60%）。
